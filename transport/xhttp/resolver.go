...
func newResolver(ctx context.Context, discovery registry.Discovery, target *Target, rebalancer selector.Rebalancer, block, insecure bool) (*resolver, error) {
	...
	if block {
		...
		case <-ctx.Done():
			r.logger.Errorf("xhttp client watch service %v reaching context deadline!", target)
			err := watcher.Stop()
			if err != nil {
				r.logger.Errorf("failed to http client watch stop: %v", target)
			}
	...
	go func() {
		for {
			...
			if err != nil {
				...
				r.logger.Errorf("xhttp client watch service %v got unexpected error:=%v", target, err)
...
func (r *resolver) update(services []*registry.ServiceInstance) bool {
	...
	if len(nodes) == 0 {
		r.logger.Warnf("[xhttp resovler]Zero endpoint found,refused to write,ser: %s ins: %v", r.target.Endpoint, nodes)
		return false
	}
